...
def crop_recommend(request):
    if request.method == 'POST':
        try:
            ...
            # Convert state and city to numeric values
            ...
            predicted_crop = model.predict([features])[0]  # Predicted Crop in English

            # Translate Prediction
            translator = Translator()
            translated_crop = translator.translate(predicted_crop, dest="mr").text

            print(f"Predicted (English): {predicted_crop}")
            print(f"Translated (Marathi): {translated_crop}")
            ...
            return render(request, 'home/crop_recommend.html', {'error': str(e)})
...
def disease_detection(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Get disease details (default in English)
            disease_details = disease_info.get(predicted_disease, disease_info["unknown"])

            # Translate details to Marathi (or any other language)
            translated_disease = translator.translate(predicted_disease.replace("_", " "), dest="mr").text
            translated_description = translator.translate(disease_details["description"], dest="mr").text
            translated_cure = translator.translate(disease_details["cure"], dest="mr").text

            # Add translated details to the context
            context = {
                "disease": translated_disease,
                "description": translated_description,
                "cure": translated_cure,
...
from django.shortcuts import render
from .models import CropPrice  # Ensure you have a model for crop prices
...
def crop_prices(request):
    if request.method == "POST":
        ...
        # Add logic to fetch crop prices (e.g., scraping or database)
        crop_prices = [
            {"crop": crop, "location": state, "price": 1500},  # Example data
        ]
        return render(request, "crop_prices.html", {"crop_prices": crop_prices})

    return render(request, "crop_prices.html", {})
